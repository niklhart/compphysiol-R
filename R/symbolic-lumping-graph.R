...
#' @returns A length two named list with entries `edges` (a data frame with columns `from`, `to` and `rate`) and `nodes` (a character vector)
...
.topo_order <- function(nodes, incoming) {
  ...
  repeat {
    ready <- nodes[
      !visited[nodes] &
      sapply(nodes, function(n) all(visited[incoming[[as.character(n)]]]))
    ]
  ...
  if (!all(visited)) stop("Cycle in condensation graph (should not happen)")
  order
}
