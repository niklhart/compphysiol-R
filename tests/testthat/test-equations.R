...
test_that("Concatenating Equations objects works", {
    eq1 <- equations(name = "co", expr = "Qadi+Qbon+Qhea+Qkid+Qliv+Qmus+Qski")
    eq2 <- equations(name = "Qliv", expr = "Qhep+Qspl+Qgut")
...
test_that("Equations can be created with both '...' and 'name'/'expr' arguments", {
    eq1 <- equations(co = Qadi + Qbon + Qhea + Qkid + Qliv + Qmus + Qski)
    eq2 <- equations(name = "co", expr = "Qadi+Qbon+Qhea+Qkid+Qliv+Qmus+Qski")
    expect_equal(eq1, eq2)
})
...
test_that("add_equation works in programmatic and interactive paths", {
    ...
    expect_equal(
        M1$equations[[1]],
        quote(Qadi + Qbon + Qhea + Qkid + Qliv + Qmus + Qski)
    )
    # Programmatic path 2
    ...
    expect_equal(length(M3$equations), 1)
    expect_equal(names(M3$equations), "co")
    expect_equal(
        M3$equations[[1]],
        quote(Qadi + Qbon + Qhea + Qkid + Qliv + Qmus + Qski)
    )
