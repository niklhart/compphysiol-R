...
test_that("initials retrieves initial concentrations/amounts if volumes are specified", {
    ...
    expect_equal(unname(A0), rep(list(units::set_units(0, "g")),4))

    expect_equal(names(C0), c("c[A, cyt]", "c[A, nuc]", "c[B, cyt]", "c[B, nuc]"))
    expect_equal(unname(C0), rep(list(units::set_units(0, "g/L")),4))
    ...
    expect_equal(unname(A02), rep(list(units::set_units(0, "g")),4))

    expect_equal(names(C02), c("c[A, cyt]", "c[A, nuc]", "c[B, cyt]", "c[B, nuc]"))
    expect_equal(unname(C02), rep(list(units::set_units(0, "g/L")),4))
})
